...
删除候选常见于:已移出路由的页面文件、未使用的兼容壳、重复的载荷识别、
过期的配置注释。**删之前逐项查四处**:

1. **源码引用**(含 deep-import —— barrel 之外还有人直接引路径);
2. **构建入口**(`esbuild.config.ts` / `vitest.config.ts` / `package.json` scripts);
3. **外部 API 使用者**(LAN 内的 agent 可能还在打旧端点);
4. **测试用途**(被当 fixture 或契约锁定)。

⚠️ **两条不要做的**:

- **兼容入口(如旧 `/agents` API)成本低,不要为了减行数贸然删。**
- **测试文件多不构成臃肿证据。** 只有确定某个测试锁定的是**过时契约**时才动它。

⚠️ **注释也算引用。** 2026-09-12 实测过一次:`isRunDispatchable` 搬去
`dispatchability.ts` 时文档注释被落在原文件,后来被当成「孤立残渣」清掉 ——
而它是那个函数**唯一的一份文档**。搬运票的逐字自证只覆盖函数体,
**覆盖不到被落在原处的注释**。
