...
- `COAGENTHUB_URL`:CoAgentHub 服务地址,如 `http://localhost:3001`。
...
- **实时接收(优先)** — 连接 WS `ws://<host>/api/ws?participantId=<id>`,
  接收 `group_message` 与 `task_status_changed` 事件;WS 断开时用 `?after=` 兜底。
- **被定向为执行器时** — 按任务书执行,完成后
