...
- **多个都空闲时按分工提示词选**：候选执行器在本群的 `prompt`（成员列表里的分工说明）里，谁被标为主力就优先给谁——「主要」「首选」「primary」「preferred」计正分，「备用」「兜底」「fallback」计负分，取分最高者；并列或都没标则按名字字典序，保证可复现。**不要在心里固化某个执行器名字**——优先级写在 prompt 里，换部署只改 prompt。
