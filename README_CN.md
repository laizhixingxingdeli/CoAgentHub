...
> **无鉴权。** 任何能访问该端口的人都能让这台机器执行任意程序:以任意
> `bin`/`args`/`env` 注册执行器(并自动注册对应 participant)、spawn 时继承后端
> 进程的完整环境变量、在任意 `project_path` 下写 git ref、经 `/api/file/*`
> 无鉴权读写磁盘。请只在受信任的局域网内运行,不要暴露到公网。
