...
describe("classifyQuotaFailure:结构证据与自指排除(伪额度回显修复)", () => {
  ...
  it("非零退出码 + rate limit → 仍配额(验收 3)", () => {
    ...
    expect(classifyQuotaFailure(["usage limit"], { exitCode: 1 }).isQuota).toBe(
      true,
    );
  });
  ...
  it("样本 2: 执行器额度限制 + resets around 18:33 UTC+8 → quota, cooldown 至明天 18:33 UTC+8", () => {
    ...
    vi.setSystemTime(now);
    try {
      const line = "exit 0(执行器额度限制,预计 ... resets around 18:33 UTC+8)";
      const verdict = classifyQuotaFailure([line], {
        exitCode: 0,
        taskBook: "无关内容",
      });
  ...
  it("验收 1–5 + 单位表 + 关键词优先级,同一张表驱动用例表", () => {
    ...
    try {
      const cases: ReadonlyArray<{
        ...
        {
          line: "429 请求过于频繁,请稍后重试",
          exitCode: 0,
          kind: "transient",
          note: "验收1 中文瞬时形态(无恢复信息→兜底动词)",
        },
        // 验收 2:中文相对时长跨阈值两侧(同一结构,只差单位)。
        {
          line: "429 触发限流,请 10 秒后重试",
          exitCode: 0,
          kind: "transient",
          note: "验收2 10秒 ≤ 60s",
        },
        {
          line: "429 触发限流,请 10 分钟后重试",
          exitCode: 0,
          kind: "exhausted",
          note: "验收2 10分钟 > 60s",
        },
        ...
        {
          line: "429 您的使用量已超出频率限制,将在 2026-08-30 18:03:10 重置",
          exitCode: 0,
          kind: "exhausted",
          note: "验收3 中文绝对时刻(T+1 天 2026-08-30,距 T 约 22h > 60s)",
        },
        // 对照:中文绝对时刻(上月 T−1 月 = 2026-07-30,已早于 T)→ 过去时刻仍 exhausted(R3)。
        {
          line: "429 您的使用量已超出频率限制,将在 2026-07-30 18:03:10 重置",
          exitCode: 0,
          kind: "exhausted",
          note: "对照 中文绝对时刻(T−1 月 2026-07-30,早于 T → 过去 → exhausted)",
        },
        // 验收 4:v1.0 英文用例逐字回归。
        {
          line: "[rate-limited] try again in 5 seconds",
          exitCode: 1,
          kind: "transient",
          note: "验收4 英文相对时长",
        },
        {
          line: "usage limit reached, resets around 13:33",
          exitCode: 1,
          kind: "exhausted",
          note: "验收4/R7-c 耗尽关键词先于绝对时刻",
        },
        {
          line: "429 too many requests, retrying in a moment",
          exitCode: 0,
          kind: "transient",
          note: "验收4 英文瞬时动词兜底",
        },
        ...
        {
          line: "[rate-limited] try again in 5 s",
          exitCode: 1,
          kind: "transient",
          note: "单位 s",
        },
        {
          line: "[rate-limited] try again in 5 sec",
          exitCode: 1,
          kind: "transient",
          note: "单位 sec",
        },
        {
          line: "[rate-limited] try again in 5 secs",
          exitCode: 1,
          kind: "transient",
          note: "单位 secs",
        },
        {
          line: "[rate-limited] try again in 5 second",
          exitCode: 1,
          kind: "transient",
          note: "单位 second",
        },
        ...
        {
          line: "[rate-limited] try again in 5 m",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 m",
        },
        {
          line: "[rate-limited] try again in 5 min",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 min",
        },
        {
          line: "[rate-limited] try again in 5 mins",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 mins",
        },
        {
          line: "[rate-limited] try again in 5 minute",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 minute",
        },
        {
          line: "[rate-limited] try again in 5 minutes",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 minutes",
        },
        {
          line: "429 触发限流,请 5 分后重试",
          exitCode: 0,
          kind: "exhausted",
          note: "单位 分",
        },
        {
          line: "429 触发限流,请 5 分钟后重试",
          exitCode: 0,
          kind: "exhausted",
          note: "单位 分钟",
        },
        ...
        {
          line: "[rate-limited] try again in 1 h",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 h",
        },
        {
          line: "[rate-limited] try again in 1 hr",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 hr",
        },
        {
          line: "[rate-limited] try again in 1 hour",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 hour",
        },
        {
          line: "[rate-limited] try again in 1 hours",
          exitCode: 1,
          kind: "exhausted",
          note: "单位 hours",
        },
        {
          line: "429 触发限流,请 1 小时后重试",
          exitCode: 0,
          kind: "exhausted",
          note: "单位 小时",
        },
        ...
        {
          line: "[rate-limited] usage limit reached, try again in 5 seconds",
          exitCode: 1,
          kind: "exhausted",
          note: "R7-c 耗尽关键词先于短时长",
        },
        ...
        // 不写死本地时区,见那里的说明。
        {
          ...
          exitCode: 1,
          kind: "transient",
          note: "R7-a 绝对时刻距 now ≤ 60s",
        },
      ];
      for (const c of cases) {
        const verdict = classifyQuotaFailure([c.line], {
          exitCode: c.exitCode,
        });
