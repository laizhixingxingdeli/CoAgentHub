...
describe("findings 驱动 dispatchKind 裁定回归 (spec findings-ticket-hardcoded-to-fix-bypasses-l3)", () => {
  ...
  async function bindExecutor(participantId: string, key: string) {
    const { participant: participantTable } = await import(
  ...
  async function waitForTask(messageId: string, timeoutMs = 3000) {
    ...
    throw new Error(
      `task for message ${messageId} not found within ${timeoutMs}ms`,
    );
  }

  // helper to create an anchor task for findings review_result taskId reference
  async function createAnchorTask(
    groupId: string,
    messageId: string,
    ...
    const [row] = await testDb
      .insert(taskTable)
      .values({ groupId, messageId, executorParticipantId, status: "queued" })
  ...
  it("验收1: findings + 显式 requirement → 落库 requirement", async () => {
    ...
    const res = await postMessage(reviewer.id, group.id, {
      body: JSON.stringify({
        ...
        verdict: "findings",
        findings: [{ severity: "high", note: "需改 spec" }],
      }),
      audience: "role",
  ...
  it("验收2: findings + 不带 dispatchKind → 落库 fix 且有缺省留痕(R2)", async () => {
    const coordinator = sharedCoordinator;
    const reviewer = await register(
      `fdk-reviewer-2-${Date.now()}-${Math.random()}`,
    );
    ...
    const res = await postMessage(reviewer.id, group.id, {
      body: JSON.stringify({
        ...
        taskId: anchorTask.id,
        verdict: "findings",
        findings: [{ severity: "high", note: "小修" }],
      }),
      audience: "role",
      audienceRef: "coordinator",
      specRef: "specs/y.md",
    ...
    const summary = task.diffSummary as Record<string, unknown> | null;
    expect(summary?.dispatchKindNote).toBe(
  ...
  it("验收3: findings + 显式 fix → fix 且无缺省留痕", async () => {
    const coordinator = sharedCoordinator;
    const reviewer = await register(
      `fdk-reviewer-3-${Date.now()}-${Math.random()}`,
    );
  ...
  it("验收4: 非 findings 消息 dispatchKind 逐字不变", async () => {
    const coordinator = sharedCoordinator;
    const reviewer = await register(
      `fdk-reviewer-4-${Date.now()}-${Math.random()}`,
    );
  ...
  it("验收2-生命周期: findings 缺省 fix 进入终态后仍保留 dispatchKindNote", async () => {
    const coordinator = sharedCoordinator;
    const reviewer = await register(
      `fdk-reviewer-lc-${Date.now()}-${Math.random()}`,
    );
    ...
    expect(task.dispatchKind).toBe("fix");
    expect(
      (task.diffSummary as Record<string, unknown> | null)?.dispatchKindNote,
    ).toBe("dispatchKind 由 findings 缺省推定为 fix,未由检视者显式指定");
    ...
    const patchRes = await app.request(
      `/api/groups/${group.id}/tasks/${task.id}`,
      {
        ...
        body: JSON.stringify({
          status: "failed",
          diffSummary: { error: "lifecycle test failure" },
        }),
      },
    );
    ...
    expect(
      (after.diffSummary as Record<string, unknown> | null)?.dispatchKindNote,
    ).toBe("dispatchKind 由 findings 缺省推定为 fix,未由检视者显式指定");
    expect((after.diffSummary as Record<string, unknown> | null)?.error).toBe(
  ...
  it("验收5: PATCH 不能覆盖已落库 dispatchKind(R3)", async () => {
    const coordinator = sharedCoordinator;
    const reviewer = await register(
      `fdk-reviewer-5-${Date.now()}-${Math.random()}`,
    );
