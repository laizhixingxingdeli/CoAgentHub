...
const { __resetExecutorQueueForTests, __setRateLimitForTests } = await import("@server/lib/executor-task");
import { classifyQuotaFailure } from "@server/lib/executor-task/state";

const QUOTA_PATTERNS = ["rate limit", "quota", "429", "额度", "window exhausted", "usage limit"];
...
describe("R9 quota-failure-on-clean-exit", () => {
  ...
  afterAll(() => {
    if (origBin === undefined) delete process.env.EXECUTOR_BIN_CODEBUDDY; else process.env.EXECUTOR_BIN_CODEBUDDY = origBin;
    if (origRepo === undefined) delete process.env.COAGENTHUB_REPO_ROOT; else process.env.COAGENTHUB_REPO_ROOT = origRepo;
  ...
  async function registerParticipant(body: Record<string, unknown>) {
    const res = await app.request("/api/participants", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
    if (res.status === 409) {
  ...
  async function createGroup(participantId: string, title: string) {
    const res = await app.request("/api/groups", { method: "POST", headers: { "Content-Type": "application/json", "X-Participant-Id": participantId }, body: JSON.stringify({ title }) });
    expect(res.status).toBe(200);
    return (await res.json()) as { id: string };
  }
  async function addMember(pid: string, gid: string, mid: string, roles: string[]) {
    const res = await app.request(`/api/groups/${gid}/members`, { method: "POST", headers: { "Content-Type": "application/json", "X-Participant-Id": pid }, body: JSON.stringify({ participantId: mid, roles }) });
    expect(res.status).toBe(200);
  }
  async function postMessage(pid: string, gid: string, body: Record<string, unknown>) {
    const res = await app.request(`/api/groups/${gid}/messages`, { method: "POST", headers: { "Content-Type": "application/json", "X-Participant-Id": pid }, body: JSON.stringify(body) });
    expect(res.status).toBe(200);
    return (await res.json()) as { id: string };
  }
  async function listTasks(pid: string, gid: string) {
    ...
    expect(res.status).toBe(200);
    return (await res.json()) as Array<{ id: string; messageId: string; status: string; diffSummary: any }>;
  }
  async function waitForTaskStatus(pid: string, gid: string, mid: string, status: string, timeoutMs = 15000) {
    const deadline = Date.now() + timeoutMs;
    for (;;) {
      const tasks = await listTasks(pid, gid);
      const t = tasks.find(x => x.messageId === mid);
      if (t && t.status === status) return t;
      if (Date.now() > deadline) throw new Error(`timeout waiting ${status} got ${tasks.find(x=>x.messageId===mid)?.status}`);
  ...
  it("验收1: exit0 + 额度行 + 窗口内第三方提交 + 无声明 => 判额度并冷却", async () => {
    ...
    try {
      ...
      const msg = await postMessage(coord.id, group.id, { body: "r9验收1", audience: "participant", audienceRef: buddy.id });
      setTimeout(() => { try { execFileSync("git", ["commit", "--allow-empty", "-qm", "third party spec"], { cwd: repoDir }); } catch {} }, 500);
      const t = await waitForTaskStatus(coord.id, group.id, msg.id, "failed", 15000);
    ...
    } finally { delete process.env.FAKE_R9_THIRD_PARTY; }
  }, 30000);
  ...
  it("验收2: exit0 + 额度行 + 汇报声明提交且核实 => 不判额度、不冷却,保留 quotaMatchedButCommitFound", async () => {
    ...
    try {
      const { coord, buddy, group } = await setupGroup();
      const msg = await postMessage(coord.id, group.id, { body: "r9验收2", audience: "participant", audienceRef: buddy.id });
      const t = await waitForTaskStatus(coord.id, group.id, msg.id, "done", 15000);
      expect(t.status).toBe("done");
      const { isInCooldown } = await import("@server/lib/executor-task/state");
      expect(isInCooldown({ key: "codebuddy" })).toBe(false);
      expect(t.diffSummary?.quotaMatchedButCommitFound).toBeTruthy();
      expect(String((t.diffSummary?.quotaMatchedButCommitFound as any)?.matchedLine)).toMatch(/quota|rate limit/i);
    } finally { delete process.env.FAKE_R9_VERIFIED; }
  }, 30000);

  it("验收3: [rate-limited] auto-continuing in 3s + 零提交/无声明 => isQuota false (次闸)", async () => {
    const verdict = classifyQuotaFailure(["[rate-limited] auto-continuing in 3s…"], { exitCode: 0 });
    ...
    try {
      const { coord, buddy, group } = await setupGroup();
      const msg = await postMessage(coord.id, group.id, { body: "r9验收3", audience: "participant", audienceRef: buddy.id });
      const t = await waitForTaskStatus(coord.id, group.id, msg.id, "done", 15000);
      expect(t.status).toBe("done");
      const { isInCooldown } = await import("@server/lib/executor-task/state");
      expect(isInCooldown({ key: "codebuddy" })).toBe(false);
      expect(t.diffSummary?.quotaMatchedButTransient).toBeTruthy();
      expect(String((t.diffSummary?.quotaMatchedButTransient as any)?.matchedLine)).toContain("auto-continuing");
      expect(t.diffSummary?.quotaMatchedLine).toBeUndefined();
    } finally { delete process.env.FAKE_R9_TRANSIENT; }
  }, 30000);
  ...
  it("验收4: window exhausted — resets around HH:MM + 零产出 + exit0 => 判额度并冷却", async () => {
    ...
    try {
      const { coord, buddy, group } = await setupGroup();
      const msg = await postMessage(coord.id, group.id, { body: "r9验收4", audience: "participant", audienceRef: buddy.id });
      const t = await waitForTaskStatus(coord.id, group.id, msg.id, "failed", 15000);
      expect(String(t.diffSummary?.error)).toContain("额度");
      const { isInCooldown } = await import("@server/lib/executor-task/state");
      expect(isInCooldown({ key: "codebuddy" })).toBe(true);
      expect(String(t.diffSummary?.quotaMatchedLine)).toContain("resets around");
    } finally { delete process.env.FAKE_R9_EXHAUSTED; delete process.env.FAKE_RESETS_AT; }
  }, 30000);
  ...
  it("验收5: 非零退出路径逐字不变 (exit2 + 额度行 => 仍判额度)", async () => {
    ...
    try {
      const { coord, buddy, group } = await setupGroup();
      const msg = await postMessage(coord.id, group.id, { body: "r9验收5", audience: "participant", audienceRef: buddy.id });
      const t = await waitForTaskStatus(coord.id, group.id, msg.id, "failed", 15000);
      expect(String(t.diffSummary?.error)).toContain("额度");
      const { isInCooldown } = await import("@server/lib/executor-task/state");
      expect(isInCooldown({ key: "codebuddy" })).toBe(true);
    } finally { delete process.env.FAKE_R9_NONZERO; }
  }, 30000);

  it("验收6: 留痕区分三种结局", async () => {
    __setRateLimitForTests(300_000, QUOTA_PATTERNS);
    const { coord, buddy, group } = await setupGroup();
    const msg = await postMessage(coord.id, group.id, { body: "r9验收6-未命中", audience: "participant", audienceRef: buddy.id });
    const t = await waitForTaskStatus(coord.id, group.id, msg.id, "done", 15000);
    expect(t.diffSummary?.quotaMatchedLine).toBeUndefined();
    expect(t.diffSummary?.quotaMatchedButCommitFound).toBeUndefined();
