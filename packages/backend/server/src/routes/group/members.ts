...
app
  .post(
    ...
    async (c) => {
      ...
      // 轻量能力提示 (ticket 17 + R3): 已知能力与角色匹配提示 + 角色对应 skill
      // 未装提示,绝不硬性拒绝 —— 无提示时为 null,响应形状始终带 capabilityHint
      // 字段。R3 起不再向群消息流发 skill 安装引导(接入时已投递全套 skill,
      // 且引导不再污染群时间线),提示只出现在本响应里,不产生任何群消息。
