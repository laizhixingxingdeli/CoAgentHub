 ...
 *
 * 仅以真实换行边界构成行:chunk 一律裸拼接(prev + chunk),不在 chunk 边界
 * 补 \n。流式 chunk 常在词中间切开(如 `[tool→ read_fi` + `le] ok\n`),在
 * 边界补换行会把一条真实动作行切成「词中间碎片」与半截 [tool→ 行;裸拼接
 * 让未以换行结尾的 chunk 与后续 chunk 自然接续,直到真实 \n 才成为完整行。
 * 截断只作用于已构成的行,不负责造行。
 */
