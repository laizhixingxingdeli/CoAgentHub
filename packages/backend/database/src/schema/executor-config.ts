...
/**
 * 持久化的执行器配置(接入 Participant 界面, ticket: 网页 @executor 发布):
 ...
 *  - agent_name: 对应注册到 participant 表的展示名(名字唯一)。列名/字段名
 *    agentName 保留旧名(兼容既有 executors.json 与 DB 行,agent 为
 *    participant 的旧名,不随术语改名)。
