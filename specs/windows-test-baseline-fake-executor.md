...
> **状态**: Landed — L3 通过(2026-09-08),实现 `ed7e2428`(Pi 直连实施,
> 检视者完成 L2/L3)。
> **L2 独立复核**(检视者自跑,与执行器报数逐字一致):
> 改前 `39 failed | 14 passed (53)` / 486s → 改后 **`9 failed | 44 passed (53)` / 239s**
> —— **30 条转绿,耗时减半**。硬约束通过:零生产代码改动、`.sh` 脚本内容一行未改。
>
> **本票最有价值的产出是 R3 的残留红分类,它挖出两个真实生产缺陷**:
> - **进程树终止**(残留 #1/#2/#9):`process.kill(-pid)` 在 win32 杀不掉 detached
>   的 `sh`/`sleep`,任务卡在 running;#9 的 `rmSync` EPERM 是残留进程占着 git 目录。
> - **`/tmp` 路径语义**(残留 #6/#7/#8):Node 写 `C:\tmp\...`,而 Git sh 的 `/tmp`
>   是 MSYS 临时目录,`cp` 报 `cannot stat '/tmp/...'`。
>   ⚠️ 该项**只咬 shell 脚本假执行器**;真实执行器(atomcode/codebuddy/pi)都是
>   Node CLI,解析 `/tmp` 与 server 一致 —— 平台链路不受影响(R1 轮次实测为证)。
> - 残留 #3/#4/#5 为套件污染(级联自 #1),**单独跑全绿**。
>
> 两者均属报告 R10 的另两项,各自另票;本票按红线未改生产代码。
> 后续 20 个测试文件的迁移可机械复用本票的助手。
> **版本**: 1.0
> **日期**: 2026-09-08
