...
> **状态**: Landed(第一批 `1b428c7e` + 第二批 `6b571f30`,
> 2026-09-08 检视者 L3 通过)
> **版本**: 1.1
> **日期**: 2026-09-08
>
> **L3 收口记录**:四文件基线 `2 failed | 108 passed (110)`;
> 后三个文件已从第一批后的 `11 failed` 回到 `1 failed`(既有超时红)。
> 剩下那条 `executor-quota-redispatch` 的「普通崩溃」经查**不属本票** ——
> 是缺陷 B 修复自身的回归,已另立
> [rollback-restore-fails-on-empty-checkpoint-tree.md](rollback-restore-fails-on-empty-checkpoint-tree.md)
> 并 Landed(`c05dbe35`)。
>
> **第二批的做法值得记下**:执行者没有机械地把期望改成 `null`,
> 而是**给样本补 `UTC+8` 标记**,让用例继续测它原本要测的东西
> (解析落点、冷却落库)。只有意图确实变了的才改期望。
>
> **执行者回提的三条批评,检视者确认全部成立**:
> ① 四文件清单与「后三个 1 failed」口径不一致(已由上述另票厘清);
> ② R7 表驱动整表仍绑本地 `setSystemTime`,非 UTC+8 机器上仍会飘 —— **未解决,值得另票**;
> ③ §2 里 orphan 的「若受影响」应改为确定项(实测确实受影响)。
>
