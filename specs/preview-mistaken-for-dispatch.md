...
> **状态**: Landed — L3 通过(2026-09-02),实现 `cc436c33`
> **版本**: 1.0
...
3. **端到端**:协调任务终态时,**子任务数 > 0 或工作树确有可归因提交**;
   二者皆无即为本缺陷复发。⚠️ 拿真实一轮验,不构造。

   > **判据订正(2026-09-02,L3 后)**:原文写作「子任务数 > 0」,**是错的**。
   > 兼任实现是 R3 明文允许的合法出口,同样产生 0 子任务。
   > 区分「把预览当下发」与「合法兼任」的是**有没有产出**,不是有没有子任务 ——
   > 前者什么都不会发生(如 `01a05e17-7b49`),后者有提交。
   > 实跑印证:第 9 张 `01a0603f-e952` 子任务 0 但提交 `de9a1c7f` 正确,
   > 属合法兼任,不构成复发。
