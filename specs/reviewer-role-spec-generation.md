...
> **版本**: 4.0（新增 §3.14.6：模式与工作类型拆成两根独立的轴——L3 只在
> 「三方在场 AND 本票为需求」时运行，取消两层模式下的 L3 自审，推翻 §3.14.4
> 的「逐项选模式」表述。3.9 / 3.8 原文均见下）
>
...
|---|---|---|
| **需求** | 检视者写 spec → L2 协调者 → **L3 检视者** | 在场方兼写 spec → L2 → **无 L3** |
| **修复** | 检视者分流、复用既有 specRef → L2 → **无 L3** | 同左 → L2 → **无 L3** |

**L3 只活在一个格子里。** 判定条件收敛成单个布尔式：
...
后两者必须分开：「因为是修复」是设计决定，「因为群里没检视者」是编制状况。
混成一个「不适用」就抹掉了一条可行动信息。
