...
> **状态**: Landed(`73392367`,2026-09-08 检视者 L3 通过)
> **版本**: 1.1
>
> **L3 收口记录(检视者独立复核)**:
> - **机制选得比 spec 预设的更安全**:把 0028 改成 **no-op 并保留文件**,
>   而不是「新增一条迁移按播种值匹配后删除」。
>   后者是 spec §3 R1 列的备选,也是 R2 硬约束最担心的路径 ——
>   **no-op 方案根本不存在能删掉用户行的代码路径**。
>   既有安装因 0028 早已 applied、drizzle 不重跑,用户行天然不受影响。
> - **R2 硬约束**:执行者在**生产库副本**上跑迁移,前后逐字对比 `IDENTICAL`。
>   检视者另行直查**生产库本体**:4 行俱在,
>   `codebuddy` / `codex` / `executor` / `pi` 的 `bin` 全是用户改写过的真实
>   `.cmd` 路径,**未被动**。
> - **隔离回归对照(检视者自己做的)**:`test/executor-trigger.test.ts`
>   改前 `8 failed | 15 passed (23)`,改后**完全相同**。**零回归。**
>   抽验四文件合计 `9 failed`,拆开是 8(executor-trigger 既有)
>   + 1(`check-bin` Windows 执行位,既有),无新增。
>   ⚠️ 执行者因 grep 出 32 个文件、全量定向跑超时而**未能取到改前基线** ——
>   这个缺口由检视者用上述隔离对照补上。
> - **R5**:`Local User` 保留,注释写明了例外理由。
> - **R4**:前端空态用 `data-testid="executors-empty-state"` + 渲染断言
>   (不是「函数返回了空数组」),符合验收 3 的要求。
>
> **执行者指出票面一处过时,成立**:spec 写「当前 5 行」,
> 本机实为 **4 行** —— 多了用户自建的 `pi`,少了检视者今晚为止损而删掉的
> `claude`(见 [dispatch-must-not-spawn-the-reviewer.md](dispatch-must-not-spawn-the-reviewer.md))。
> 三行播种出身且被用户改写 `bin` 的(`executor`/`codebuddy`/`codex`)全部健在。
>
> **v1.1 修订(2026-09-08,检视者在派发前复核发现)**:
> §4.4 的回归口径写的是全量 `npx vitest run`,并引了一个已经过时的红基线。
> 更正为「先 grep 出受影响文件 → 用 `scripts/test-baseline.mjs` 取定向基线」。
> 详见该条内的说明。
>
> **日期**: 2026-09-07
...
4. **回归**(v1.1 更正,见下):**只跑定向清单,不跑全量。**

   > ⚠️ **v1.1 更正(2026-09-08,检视者)**:本条原写
   > `cd packages/backend/server && npx vitest run`(全量),
   > 并引 2026-09-07 的 **129 failed | 924 passed** 作基线。
   > **两处都过时了**:
   > ① 用户 2026-09-07 已定口径「**不跑全量,只跑修改部分**」——
   >    本机全量约 24 分钟,一张票跑一次是纯浪费;
   > ② 那个基线也不再成立 —— 跨平台假执行器迁移(B1–B5)之后
   >    协调者实测为 **26 failed | 1048 passed**。
   >
   > 与 `specs/ticket-template-demands-impossible-green.md` 同源的错误。

   **正确做法**:先 `grep -rl 'seedBuiltinExecutorConfigs\|0028' packages/backend/server/test/`
   把受影响的测试文件全找出来(⚠️ **这一步不能省** ——
   R11 与缺陷 B 都因为漏了它而留下清单外的红),再:

   ```
   node scripts/test-baseline.mjs packages/backend/server <找出来的文件...>
   ```

   改前改后各取一行,口径**失败数不增加**。
   若有测试依赖播种行,按新契约更新并**逐条列出改了哪几个、为什么**。
