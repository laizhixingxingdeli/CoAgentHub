...
**verdict: pass**,commit `b30a4a9`。

检视者独立核实 `test/executor-ansi.test.ts` —— spec 的硬要求「只测单 chunk 不算数」**做到了**,且覆盖两种切法:

```
跨 chunk 切法一:  strip("\u001b[3") → ""           (前半块扣住不发)
                  strip("2mgreen\u001b[0m") → "green"  (补齐后一起剥)
跨 chunk 切法二:  前块只留 ESC,后块以 '[' 开头补齐
```

另核对:唯一源头剥离、`process.stdout.write` 保留原始 chunk、共享 ANSI 正则、`output-buffer` 注释已修正。
