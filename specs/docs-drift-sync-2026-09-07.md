...
> **状态**: Landed — L3 通过(2026-09-07),实现 `e3ed2cb8`(Pi 直连实施,
> 检视者完成 L2/L3)。七条逐条复核结果:
> D-1/D-2/D-4 已改且口径正确(D-4 三处一致);
> D-3、D-7 **原决策正文一字未动**,只在尾部追加「后续演进 / 后续决策」——
> diff 里只有 `+` 行可证;
> D-5、D-6 判定为**无需改动**并附检索依据(未发现相关表述),符合本 spec
> 「没有就不要新造」的处置;
> 硬约束通过:提交 5 个文件全为文档,零 `.ts`/`.sql`/`.json`/`.mjs`,未跑测试。
> **版本**: 1.0
> **日期**: 2026-09-07
